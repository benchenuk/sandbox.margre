...
from margre.workflow.synthesis import synthesis_node
...
def continue_to_researchers(state: OrchestratorState) -> list[Send]:
    ...
    logger.info(
        f"ORCHESTRATOR: Dispatching {len(sends)} discovery agents for '{plan.seed_person}'."
    )
    return sends
...
) -> Literal["researcher_dispatch", "planner"]:
    """
...
def route_after_candidate(state: OrchestratorState) -> Literal["expand", "end"]:
    """
    ...
    if loop_count >= config.workflow.max_expansion_depth:
        logger.info(
            f"ORCHESTRATOR: Expansion depth limit ({config.workflow.max_expansion_depth}) reached."
        )
        return "end"
    ...
    logger.info(
        f"ORCHESTRATOR: {len(state['suggested_gaps'])} candidates ready for expansion loop {loop_count + 1}."
    )
    return "expand"
...
def create_graph(checkpointer: SqliteSaver | None = None):
    ...
    workflow.add_conditional_edges(
        "planner_node",
        route_after_planner,
        {"researcher_dispatch": "research_dispatch_node", "planner": "planner_node"},
    )

    workflow.add_conditional_edges(
        "research_dispatch_node", continue_to_researchers, ["researcher_node"]
    )

    workflow.add_edge("researcher_node", "synthesis_node")
    workflow.add_edge("synthesis_node", "candidate_node")

    workflow.add_conditional_edges(
        "candidate_node", route_after_candidate, {"expand": "planner_node", "end": END}
    )

    # Compile with checkpointer and HITL interrupts
    return workflow.compile(
        checkpointer=checkpointer, interrupt_after=["planner_node", "candidate_node"]
    )
