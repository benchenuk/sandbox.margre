...
def planner_node(state: OrchestratorState) -> dict:
    ...
    logger.info(
        f"PLANNER: Analyzing connections for: {seed_person} (Loop: {loop_count})"
    )
    ...
    logger.info(
        f"PLANNER: Found {len(connections)} existing connections for {seed_person} in graph."
    )
    ...
    if revision_comments is not None:
        ...
        if current_plan and current_plan.subtasks:
            ...
            system_content = DISCOVERY_PLANNER_SYSTEM_PROMPT.format(
                seed_person=seed_person
            )
    elif loop_count > 0 and state.get("suggested_gaps"):
        logger.info(
            f"PLANNER: Refining discovery based on {len(state['suggested_gaps'])} gaps."
        )
        gap_names = [
            c.name if hasattr(c, "name") else c for c in state["suggested_gaps"]
        ]
    ...
    try:
        ...
        subtask_summaries = ", ".join(
            [f"{t.target_person} ({t.search_angle})" for t in plan.subtasks]
        )
        logger.info(
            f"PLANNER: Generated discovery plan with {len(plan.subtasks)} subtasks: {subtask_summaries}"
        )
        ...
        if revision_comments:
            revision_count += 1
            logger.info(
                f"PLANNER: Revised plan with {len(plan.subtasks)} subtasks (revision #{revision_count})"
            )
        ...
        return {
            ...
            "messages": [
                HumanMessage(
                    content=f"Discovery plan generated with {len(plan.subtasks)} tasks"
                )
            ],
        ...
        logger.warning(
            f"PLANNER: Structured output failed (Error: {e}), falling back to manual JSON parsing..."
        )

        fallback_prompt = [
            SystemMessage(
                content=PLANNER_FALLBACK_SYSTEM_PROMPT.format(seed_person=seed_person)
            ),
            HumanMessage(
                content=f"Identify discovery tasks for the social and professional network of: {seed_person}"
            ),
        ]
        ...
        try:
            ...
            if "seed_person" not in plan_dict:
                plan_dict["seed_person"] = seed_person
            ...
            logger.error(
                f"PLANNER: Fallback JSON parsing also failed: {json_err}. Raw response: {raw_res}"
            )
            raise e
