...
class DiscoveryTask(BaseModel):
    ...
    search_angle: str = Field(
        description="The perspective (collaborators, critics, institutions, influence)"
    )
    research_query: str = Field(
        description="Specific research question or query to pursue"
    )
...
class DiscoveryPlan(BaseModel):
    ...
    subtasks: List[DiscoveryTask] = Field(
        description="Connection-oriented discovery tasks"
    )
...
class DiscoveredRelationship(BaseModel):
    ...
    rel_type: str = Field(
        description="Relationship label (e.g., KNEW, COLLABORATED_WITH, STUDIED_AT)"
    )
    target_name: str = Field(
        description="Name of the other person, institution, contribution, event, or location"
    )
    target_label: str = Field(
        description="Type of the target entity (Person, Institution, Contribution, Location, Event)"
    )
    context: str = Field(description="Brief historical context of the connection")
    year: Optional[int] = Field(
        None, description="The most relevant year for timeline ordering"
    )
    date: Optional[str] = Field(
        None, description="Specific known date (e.g., DoB, death date)"
    )
    period: Optional[str] = Field(
        None, description="Human readable period or date range"
    )


class Candidate(BaseModel):
    """A candidate person for expansion with ranking info."""
...
class DiscoveryExtractionResult(BaseModel):
    ...
    new_persons: List[str] = Field(
        default_factory=list,
        description="List of NAMES of and only of individuals for potential further research",
    )
